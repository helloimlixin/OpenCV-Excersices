        ...
        for j in np.arange(0, src.shape[1] - temp.shape[1], stepsize):
            ...
            for x in range(0,temp.shape[0]):
                for y in range(0,temp.shape[1]):
                    corr += (src[x+i,y+j] - mean_s) * (temp[x,y] - mean_t)
...
# directory should be changed accordingly while running
source_img = cv2.imread('/Users/xinli/Desktop/EC601_OpenCV/source_img.jpg',0) # read image in grayscale
temp = cv2.imread('/Users/xinli/Desktop/EC601_OpenCV/template_img.jpg',0) # read image in grayscale
...
# ------------------ Put your code below ------------------ 
cv2.imwrite('result_image.jpg',match_img)
